...
static float sPerspectiveMatrix[16] = {0};
...
initializeProgram()
{
    ...
    sPerspectiveMatrix[0] = skFrustumScale;
    sPerspectiveMatrix[5] = skFrustumScale;
    sPerspectiveMatrix[10] = (skZFar + skZNear) / (skZNear - skZFar);
    sPerspectiveMatrix[11] = -1.0f;
    sPerspectiveMatrix[14] = (2.0f * skZFar * skZNear) / (skZNear - skZFar);

    glUseProgram(sTheProgram);
    glUniformMatrix4fv(sPerspectiveMatrixUniform, 1, GL_FALSE, sPerspectiveMatrix);
    glUseProgram(0);
...
{
    sPerspectiveMatrix[0] = skFrustumScale / ((float)width / height);
    sPerspectiveMatrix[5] = skFrustumScale;
...
gltutDisplay(void)
{
    ...
    glUseProgram(sTheProgram);
    glUniform2f(sOffsetUniform, 1.5f, 0.5f);
